...
public final class CameraManager {
    ...
    private static int MAX_FRAME_WIDTH = 540; // = 6/8 * 720
    private static int MAX_FRAME_HEIGHT = 960; // = 6/8 * 1280

    private static final float FORM_SQUARE = 5 / 8f;
    private static final float FORM_RECTANGLE_WIDTH = 3 / 4f;
    private static final float FORM_RECTANGLE_HEIGHT = 1 / 4f;

    public boolean orientation = false; // 动画方向，true=竖向扫描动画，false=横向扫描动画
    ...
    public CameraManager(Context context) {
        ...
        MIN_FRAME_WIDTH = MIN_FRAME_HEIGHT = Math.min(dm.widthPixels, dm.heightPixels) * 1 / 4;
        MAX_FRAME_HEIGHT = MAX_FRAME_WIDTH = Math.min(dm.widthPixels, dm.heightPixels) * 5 / 8;
    }
    ...
    public synchronized Rect getFramingRect() {
        if (framingRect == null) {
            ...
            int width, height;
            if (orientation) {
                width = findDesiredDimensionInRange(screenResolution.x, MIN_FRAME_WIDTH, MAX_FRAME_WIDTH, FORM_SQUARE);
                ...
                width = findDesiredDimensionInRange(screenResolution.x, MIN_FRAME_WIDTH, MAX_FRAME_WIDTH,
                        FORM_RECTANGLE_WIDTH);
                height = findDesiredDimensionInRange(screenResolution.y, MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT,
                        FORM_RECTANGLE_HEIGHT);
            }

            int leftOffset = (screenResolution.x - width) / 2; // 扫描窗口的左上角锚点之x
            int topOffset = (screenResolution.y - height) / 2; // 扫描窗口的左上角锚点之y
    ...
    private static int findDesiredDimensionInRange(int resolution, int hardMin, int hardMax, float radio) {
        int dim = Math.round(resolution * radio); // Target 5/8 of each dimension
        if (dim < hardMin) {
            return hardMin;
        }
        if (dim > hardMax) {
